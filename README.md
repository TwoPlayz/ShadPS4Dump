...
Drop-in Windows DLL that restores **File → Install Game PKG** to [shadPS4QtLauncher](https://github.com/shadps4-emu/shadps4-qtlauncher) without modifying the emulator.

## Quick start

1. Build `version.dll` (see [BUILD.md](BUILD.md)) or download a release artifact.
2. Copy `version.dll` next to `shadPS4QtLauncher.exe`.
3. Launch the Qt launcher — **Install Game PKG...** appears in the **File** menu.
...
| **Menu** | Qt 6 draws the menu bar (not Win32 `HMENU`). The plugin injects **Install Game PKG...** into the Qt **File** menu on the GUI thread. Win32 `GetMenu` is tried first as a fallback. |
...
- **Split patch PKGs** — select all `piece_*.pkg` files in one install; they are merged automatically.
